...
class DS2438 
{
    public:
		...
		void begin(float coefficientA, float offsetA, float coefficientB, float offsetB);
        void update();
        double getTemperature();
		float getRawData(bool channel);
		float getCurrent(bool channel);
		void setCoefficient(bool channel);
		float getCoefficient(bool channel);
    private:
        OneWire *_ow;
        uint8_t *_address;
        double _temperature;
